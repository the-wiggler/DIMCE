...
class PlotArrays:
    ...
    def stdvhistory(self):
        ...
        plt.plot(x_fit, y_fit, color='red', label=fr'Fitted curve: $y = \frac{{{model.params[1]:.3f}}}{{x}}$ + {{model.params[0]:.3f}}')
        plt.ylim(0, y.max())
        plt.legend()
...
plot_arrays = PlotArrays(
    avg_calc_int=np.load(os.path.join('np_store_CPU', 'avg_calc_int_CPU.npy')),
    std_list=np.load(os.path.join('np_store_CPU', 'std_list_CPU.npy')),
    history_count_list=np.load(os.path.join('np_store_CPU', 'history_count_list_CPU.npy')),
    batch_times=np.load(os.path.join('np_store_CPU', 'batch_times_CPU.npy')),
)